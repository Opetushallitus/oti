...
CREATE TYPE registration_state_new AS ENUM ('OK', 'INCOMPLETE', 'CANCELLED', 'ABSENT', 'ABSENT_APPROVED');

ALTER TABLE registration
  ALTER COLUMN state
    DROP NOT NULL;

UPDATE registration SET state = null WHERE state = 'ERROR'::registration_state;

ALTER TABLE registration
  ALTER COLUMN state TYPE registration_state_new
    USING (state::text::registration_state_new);

DROP TYPE registration_state;

ALTER TYPE registration_state_new RENAME TO registration_state;

UPDATE registration SET state = 'CANCELLED'::registration_state WHERE state IS NULL;
