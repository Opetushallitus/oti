CREATE OR REPLACE VIEW all_participant_data AS
  SELECT
    p.id, p.ext_reference_id, email, s.id AS section_id, st.name AS section_name, m.id AS module_id,
    ...
    r.id AS registration_id, r.language_code AS registration_language
  FROM participant p
    ...
    LEFT JOIN registration_exam_content_module recm ON recm.module_id = m.id AND recm.participant_id = p.id AND recm.registration_id = recs.registration_id
    LEFT JOIN registration r ON recs.registration_id = r.id
    LEFT JOIN exam_session es ON es.id = r.exam_session_id
    LEFT JOIN payment pm ON p.id = pm.participant_id AND (pm.registration_id = r.id OR (pm.registration_id IS NULL AND r.id IS NULL))
